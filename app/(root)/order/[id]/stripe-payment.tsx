  ...
  const StipeForm =()=>{
    ...
    return (
        <form className='space-y-4' onSubmit={handleSubmit}>
            ...
            <div>
                <LinkAuthenticationElement onChange={(event)=>{setEmail(event.value.email)}}></LinkAuthenticationElement>
            </div>
            <Button className='w-full' size='lg' disabled={stripe == null || elements == null || isLoading}>
                {isLoading ? 'Making your purchase, hang tight': `Purchase ${formatCurrency(priceInCents/100)}`}
            </Button>
